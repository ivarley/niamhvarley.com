...
- Plain static site. HTML + CSS + a small amount of vanilla JS. No framework. Every page
  is generated by `build.js` — plain Node, no dependencies — and the generated HTML is
  committed, so deploying is still dropping a folder on Netlify / GitHub Pages / S3 with
  no build at serve time. Never hand-edit the generated `.html`; see `README.md`.
...
- **For grown-ups** — one page: the art background, how to coach without wrecking it, the
  book recommendations. Source: `GROWNUPS.md`. (`ART-NOTES.md` is the raw material that
  page was written from — nothing reads it.)
